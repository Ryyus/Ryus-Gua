...
- 六爻与塔罗主页均可选写下占问；不填写可照常占卜，填写后与本局历史一同保存在本机。
- 占问只在用户主动调用 AI 解读时发送给其自行配置的服务商；六爻与塔罗使用完全独立的系统提示词。
- AI 正文建议 350–550 个中文字符，必要时最多 650 个字符；设置中可按服务商选择模型默认、低、中、高思考强度。
- 塔罗离线合参会分析大牌比例、花色重心、正逆位结构、牌位联系以及三牌/四象的不同组织方式。
- 自绘界面新增虚拟无障碍按钮、屏幕摘要、48dp 触控范围、字体缩放与紧凑屏适配。
- `MainActivity` 已拆成轻量 Activity 壳与独立 `GuaView`，提示词、合参、无障碍也分别独立维护。
- 版本号提升至 **1.8.0 / versionCode 180**；正式稳定版标记仍保持 1.7.0，直至 1.8.0 通过验证并发布。
