...
final class TarotHistoryStore {
    ...
    static final class Entry {
        ...
        final String question,note,aiText,aiReasoning,aiModel; final boolean aiReasoningSummaryOnly;
        Entry(String id,long time,String spread,int[] cards,boolean[] reversed,boolean pinned,String question,String note,String ai,String reasoning,boolean summaryOnly,String model){
            this.id=id==null||id.isEmpty()?UUID.randomUUID().toString():id;timeMillis=time;spreadId=spread==null||spread.isEmpty()?"three":spread;
            cardIds=cards.clone();this.reversed=reversed.clone();this.pinned=pinned;this.question=cap(question,120);this.note=cap(note,300);aiText=cap(ai,12000);aiReasoning=cap(reasoning,12000);aiReasoningSummaryOnly=summaryOnly;aiModel=cap(model,200);
        }
    ...
    static Entry add(Context context,String spreadId,TarotDeck.Draw[] draws,String question){
        ...
        List<Entry> entries=load(context);Entry e=new Entry(UUID.randomUUID().toString(),System.currentTimeMillis(),spreadId,ids,rev,false,question,"","","",true,"");entries.add(0,e);prune(entries);save(context,entries);return e;
    }
    static List<Entry> load(Context context){
        ...
        try{JSONArray arr=new JSONArray(p.getString(KEY,"[]"));for(int i=0;i<arr.length();i++){JSONObject o=arr.getJSONObject(i);JSONArray idsJson=o.getJSONArray("cards"),revJson=o.getJSONArray("reversed");int count=idsJson.length();if(count<1||count>6||revJson.length()!=count)continue;int[] ids=new int[count];boolean[] rev=new boolean[count];for(int j=0;j<count;j++){ids[j]=idsJson.getInt(j);rev[j]=revJson.getBoolean(j);}out.add(new Entry(o.optString("id",""),o.optLong("time",0L),o.optString("spread","three"),ids,rev,o.optBoolean("pinned",false),o.optString("question",""),o.optString("note",""),o.optString("ai",""),o.optString("aiReasoning",""),o.optBoolean("aiReasoningSummaryOnly",true),o.optString("aiModel","")));}}catch(Exception ignored){}
        sort(out);return out;
    }
    ...
    private static Entry copy(Entry e,boolean pinned,String note,String ai,String reasoning,boolean summaryOnly,String model){return new Entry(e.id,e.timeMillis,e.spreadId,e.cardIds,e.reversed,pinned,e.question,note,ai,reasoning,summaryOnly,model);}
